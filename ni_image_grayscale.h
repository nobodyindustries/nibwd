...
__ni_image_grayscale_itu_bt_601(stbi_uc r, stbi_uc g, stbi_uc b)
{
	...
	double l = (0.299 * ((double)r)) + (0.587 * ((double)g)) + (0.114 * ((double)b));
...
__ni_image_grayscale_smpte_240m(stbi_uc r, stbi_uc g, stbi_uc b)
{
	...
	double l = (0.212 * ((double)r)) + (0.701 * ((double)g)) + (0.087 * ((double)b));
	return (stbi_uc)round(l);
}
...
{
	assert(n_channels == 3);
	stbi_uc *ret_img = ni_image_create(w, h, 1); // This image is grayscale -> 1 channel
